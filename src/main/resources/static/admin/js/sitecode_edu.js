var siteCode = {
	...
	select: function (oThis, pt_code, ori_code, depth) {
		...
		$("#sitecode_" + depth).attr("ori_code", ori_code);
		...
		$.ajax({
			...
			data: "{ 'pt_code': '" + pt_code + "', 'ori_code': '" + ori_code + "', 'depth': '" + depth + "' }",
			success: function (data) { siteCode.selectResult(data, ori_code, depth); }
		});

	},

	selectResult: function (data, ori_code, depth) {
        ...
        */
		...
		html += "<td onclick='siteCode.select( this, \"{CODE}\", \"{ORI_CODE}\", " + select_depth + " );'><img src='../images/icon_accept.gif' class='select_icon' alt='선택' /></td>";	 //선택
		html += "</tr>";
		...
		for (var i = 0, j = data_arr.length; i < j; i++) {
			...
			var json = data_arr[i];
			var new_html = html.replace("{CODE_NAME}", json.code_name).replaceAll("{CODE}", json.code).replaceAll("{ORI_CODE}", json.ori_code);
			cache_html += new_html;
	...
	add: function (oThis) {
		...
		//alert("{ 'code_name': '" + code_name + "', 'pt_code': '" + pt_code + "','ori_code': '" + ori_code + "', 'depth': '" + depth + "' }");
		...
		$.ajax({
			...
			data: "{ 'code_name': '" + code_name + "', 'pt_code': '" + pt_code + "', 'ori_code': '" + ori_code + "', 'depth': '" + depth + "' }",
			success: function (data) { siteCode.selectResult(data, ori_code, depth); }
		});
