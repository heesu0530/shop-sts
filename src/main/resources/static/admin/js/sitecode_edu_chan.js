var siteCode = {
	...
	selectResult: function (data, ori_code, depth, table_name) {
		...
		html += "<td class='tx_a_left'><span onclick='siteCode.modifyClick( this, \"" + table_name + "\" );'>{CODE_NAME}</span></td>";		                                                     //코드명
	    //치매센터용일떄만
		if (get_table == "A") {

		    html += "<td><img src='../images/cog.gif' alt='설정' onclick='FrameLayer.open( \"./regiForm.aspx?code={CODE}\", 550, 530, \"no\" );' /></td>";
		}
	...
	modifyClick: function (oThis, table_name) {
	...
	modify: function (oThis, table_name) {
	  ...
	  var tr = $(oThis).parents("tr");							            //Tr 개체
		...
		$.ajax({
			...
			data: "{ 'code': '" + code + "', 'code_name': '" + code_name + "', 'table_name': '" + table_name + "' }",
